...
class AdminViews:
    ...
    def get_fields(instancia):
        ...
        field_values = []
        for field in fields:
            if field.concrete and not field.is_relation and field.name not in AdminViews.fields_exclude[instancia.__class__.__name__.lower()]:
                field_values.append({
                    'name': field.name,
            ...
            elif field.is_relation and field.name not in AdminViews.fields_exclude[instancia.__class__.__name__.lower()]:
    ...
    def listar_instancias(request, classe):
        ...
        return render(request, 'elibrosLoja/admin/manter.html',
                    ...
                    'classe': classe,
                    'permissoes': AdminViews.permissoes_admin_elibros[classe],
                    'botao': botao, 
    ...
    @login_required
    def editar_instancia(self, request, classe, id):
        if classe in ['livro', 'genero', 'categoria', 'cliente', 'pedido', 'cupom']:
            instancia = eval(classe.capitalize()).objects.get(id=id)
            h2 = AdminViews.get_h2(classe, instancia)
            field_values = AdminViews.get_fields(instancia)
    ...
    def criar_instancia(request, classe):
        if request.method == 'POST':
            ...
            return render(request, 'elibrosLoja/admin/criar_instancia.html', {'classe': classe})
