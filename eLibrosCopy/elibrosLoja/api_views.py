...
# ACHO QUE ISSO ABAIXO NÃO VAI SER USADO
...
@api_view(['GET'])
@permission_classes([AllowAny])
def livros_destaque(request: Request) -> Response:
...
@api_view(['GET'])
@permission_classes([AllowAny])
def livros_lancamentos(request: Request) -> Response:
...
def avaliacoes_livro(request: Request, livro_id: int) -> Response:
    """
    ...
    # Retorno padrão para métodos não implementados
    return Response({'detail': 'Método não permitido'}, status=status.HTTP_405_METHOD_NOT_ALLOWED)


@api_view(['POST', 'DELETE'])
@permission_classes([IsAuthenticated])
def curtir_avaliacao(request: Request, avaliacao_id: int) -> Response:
    """
